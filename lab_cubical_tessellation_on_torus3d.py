...
from package.tessellations import get_cubical_tessellation_on_torus3d
...
filename = 'lab-results/lab_cubical_tessellation_on_torus3d/size{0}_exp{1}.csv'

for size in sizes:
    bar0.set_postfix_str('size='+str(size))
    fc = get_cubical_tessellation_on_torus3d(n=size)
    filtrations = np.random.random([experiments, size**3])
