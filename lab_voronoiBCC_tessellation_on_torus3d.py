...
from package.tessellations import get_bcc_tessellation_on_torus3d



clock = time.perf_counter()

sizes = np.arange(3, 64)
experiments = 100
...
filename = 'lab-results/lab_voronoiBCC_tessellation_on_torus3d/size{0}_exp{1}.csv'

for size in sizes:
    bar0.set_postfix_str('size='+str(size))
    fc = get_bcc_tessellation_on_torus3d(n=size)
    filtrations = np.random.random([experiments, 2*size**3])
